...
/** 冻结设计 §2 七态（LOCALLY_REJECTED 不入 journal，故无此枚举值）。 */
enum class CommandStatus {
    QUEUED,      // Body 已持久保存
    REJECTED,    // 过期/策略拒绝（receipt 留痕）
    ACCEPTED,    // Brain 原子领取（prompt_start）
    WAITING_USER,// 暂停/阻塞/HITL
    RESOLVED,    // 持久终态
    INTERRUPTED, // 已接受但 Brain 崩溃、副作用边界未知——禁自动重放
}
...
class AndroidSqliteCommandDb(private val helper: SQLiteOpenHelper) : CommandDb {
    ...
    private fun contentValues(r: CommandRecord): ContentValues = ContentValues().apply {
        ...
        put("kind", r.kind)
        put("text", r.text) // journal 本体需要 brain 重放原文；「普通日志不写明文」约束的是 logcat/审计日志
        ...
        fun createTable(db: SQLiteDatabase) {
            db.execSQL(
                "CREATE TABLE IF NOT EXISTS commands (" +
                    ...
                    "kind TEXT NOT NULL, text_sha TEXT NOT NULL DEFAULT '', text TEXT NOT NULL," +
...
class CommandStore(private val db: CommandDb) {
    ...
    fun reserve(kind: String, text: String, ttlMs: Long = 10 * 60 * 1000): ReserveOutcome {
        ...
        db.insert(
            CommandRecord(
                id = 0, commandId = commandId, kind = k, text = t,
    ...
    /**
     * Brain 原子领取（冻结设计 §4：atomic claim）：
     * - QUEUED 且未过期 → ACCEPTED（记 brainSession；taskId 由 prompt_start 后 mark）
     * - QUEUED 已过期 → REJECTED（receipt=expired）
     * - 其他状态（已被领/已终态）→ 拒绝
     */
    sealed interface ClaimOutcome {
        data class Accepted(val commandId: String) : ClaimOutcome
        data class Rejected(val reason: String) : ClaimOutcome
    }

    fun claim(commandId: String, brainSession: String): ClaimOutcome {
        val rec = db.findByCommandId(commandId) ?: return ClaimOutcome.Rejected("commandId 不存在")
        val now = System.currentTimeMillis()
        if (rec.status == CommandStatus.QUEUED) {
            if (rec.expiresAt in 1 until now) {
            ...
            if (db.compareAndUpdateStatus(commandId, CommandStatus.QUEUED, CommandStatus.ACCEPTED, now, brainSession = brainSession)) {
                return ClaimOutcome.Accepted(commandId)
            }
            return ClaimOutcome.Rejected("并发领取竞争失败（他方已领）")
        }
        return ClaimOutcome.Rejected("当前状态 ${rec.status} 不可领取")
    }

    /** 终态/中间态标记：resolve(success|failed|stopped→RESOLVED)、mark(waiting_user)、崩溃边界(→INTERRUPTED)。 */
    fun mark(commandId: String, to: CommandStatus, taskId: String? = null): Boolean {
        val rec = db.findByCommandId(commandId) ?: return false
        val now = System.currentTimeMillis()
        // 合法迁移：ACCEPTED→WAITING_USER/RESOLVED/INTERRUPTED；WAITING_USER→ACCEPTED(继续)/RESOLVED；
        // INTERRUPTED→RESOLVED（仅人工核对——冻结设计 §2：禁止自动重放）
        ...
        return db.compareAndUpdateStatus(commandId, rec.status, to, now, taskId = taskId)
    }
