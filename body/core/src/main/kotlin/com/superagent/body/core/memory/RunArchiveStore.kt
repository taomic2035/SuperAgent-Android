...
class RunArchiveStore(private val db: RunArchiveDb) {
    ...
    fun archive(
        ...
        require(outcome in OUTCOMES) { "outcome 非法：$outcome（允许 success|failed|crashed|needs_human|closed|paused|stopped）" }
    ...
    companion object {
        private val OUTCOMES = setOf("success", "failed", "crashed", "needs_human", "closed", "paused", "stopped")
