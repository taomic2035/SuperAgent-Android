...
| TC-07 | ⏳ | 语音模型已就位（2026-08-19 核实），装机+端到端执行中（docs/handoff/2026-08-19-tc07-voice-e2e.md） |
| TC-08 | ◐ | brain 侧已实现（2026-08-19）：证据驳回计数，连续 3 次驳回错误信息升级为强制 hitl.handoff 建议、成功清零、beginRun 重置（smoke 第 4 节覆盖）；真机基线证据驳回实测待补 |
| TC-11/12/13 | ⏳ | SPEECH_UNAVAILABLE(reason=model) 降级正确；语音模型已就位（2026-08-19 核实），真机质量验收待 TC-07 通路后执行 |
| TC-14 | ◐ | brain 侧已实现（2026-08-19）：启动预览未完成任务、「继续」恢复、续跑上下文基于脱敏 trace 注入 prompt、success 终态不可续（smoke 第 6 节覆盖）；真机 kill/重启续走验证待补 |
