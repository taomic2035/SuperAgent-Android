...
skill.{list,run,learn,feedback}                                          [feedback 新增 P1]
...
```
第1层 brain守卫（软，可被绕过也要有）
  beforeToolCall: selectOption/selectSpec 目标命中边界词表 → block + 提示转HITL
第2层 body守卫（硬，最高权威）
  ① 词表拦截（保留v1词表，作为信号之一）
  ② 动作语义标记：launch返回的appPackage ∈ 敏感App注册表(银行/支付/社交)时，
     body进入"敏感会话"状态，后续所有提交类动作强制HITL
  ③ WebView感知：a11y发现WebView节点且URL/内容含pay|checkout|收银 → 整页标记sensitive
第3层 HITL（人是否在场是最终权威）
  提交边界动作 → hitl.confirm（通知，60s超时默认拒绝，denial is data）
```

**词表治理**：整词/短语精确匹配（非子串），避免误伤"支付宝分享"面板。词表单一来源存 body `common/src/main/resources/commit_boundaries.json`（WP1 实施），brain 侧 `commitBoundary.ts` 从同源 JSON 读取，双侧一致性进验收清单。词表内容含：`commitPhrases`（提交边界词）、`sensitiveNavPhrases`（敏感导航词）、`sensitiveSessionActionVerbs`（敏感会话内额外确认词）、`sensitiveUrlPatterns`（WebView URL 特征）、`sensitiveAppPrefixes`（敏感App包名前缀）。
...
- brain 复用 pi-agent-core session 机制，每轮对话后落盘（Termux 私有目录）；
- `runState`（goal/baseline/trace）随会话序列化，重启恢复；
- body 侧技能库/声纹库本就落盘，无状态依赖 brain；
- 断点续跑语义：恢复后 brain 从最后 trace 步骤重新感知当前屏幕，diff 后决定续走或重规划（与 skill recovery mode 同一套机制，代码复用）。
...
│  ├─ 1.4 守卫层
│  │  ├─ 1.4.1 提交边界第一道过滤（brain侧）
│  │  ├─ 1.4.2 证据闸门（存在性+新颖性+相关性）
│  │  └─ 1.4.3 隐私脱敏（发送前打码）
│  ├─ 1.5 技能检索（中文2gram TF-IDF，与body同算法对齐）
│  ├─ 1.6 角色系统（personas：assistant/momo，VoiceConfig契约）
│  ├─ 1.7 会话持久化（session落盘 + runState断点）
