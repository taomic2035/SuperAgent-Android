...
> 文档定位：本文修订并取代 docs/04 的架构部分，是当前实现的**架构事实源**；v3 于 2026-08-21 依据 `docs/16` 持续校准，最近验证证据以 README 流水线和 CI 为准。
...
| 关注点 | brain | body | 当前缺口 |
|---|---|---|---|
| 任务决策 | Agent/模型/守卫 | typed denial | 终态单写已闭环；真实 kill/restart 待真机 |
| 设备动作 | 只发 Action 请求 | 最终坐标校验并执行 | selector 已闭环；敏感页真机回归待验 |
| 感知 | VLM 解释截图 | a11y preflight/截图权威 | attach/WebView 代码闭环；真机标定待验 |
| UI | 只发类型化事件 | UI 状态唯一 owner | pause/resume 已闭环；跨进程 command ack 待补 |
| 技能 | 召回与恢复规划 | 存储/回放/状态权威 | 多 App 与长期失配率待验 |
| 记忆 | 提取与 prompt 注入 | SQLite 权威/用户删除 | import 去重与真实恢复演练待验 |
...
| L1 | MediaProjection 截图 → 可配置视觉模型（VISION_MODEL） | WebView/Canvas/图片 | ~1-2s + 网络 | ◐ 授权回调已接（C-03），真机标定待设备 |
| L2 | L0+L1 混合路由（auto 模式） | a11y 盲区 | 混合 | ◐ WebView 信号已修（C-04 className 结构化），真机验证待设备 |

当前 screenshot/blob/VLM、MediaProjection attach、WebView auto、typed fallback 与旋转 fail-closed 已形成代码闭环；完成真机授权生命周期和坐标标定前，仍不得宣称设备侧视觉全线验收。
...
| 档 | 模型 | 能力承诺 | 触发条件 |
|---|---|---|---|
| M1 | 可配置云端主模型（MODEL，历史默认 GLM-4.6v） | 完整操控闭环（tool-calling + 视觉） | 默认 |
| M2 | 备用云端供应商（OpenAI 兼容） | 完整操控闭环（无视觉或降级视觉） | 主 provider 不可达、限流或能力不满足 |
...
已实现：protocolVersion、typed 错误、请求体 10MB 上限、512 条 FIFO 幂等缓存、同 key in-flight 合并、结构化 handler 超时取消、GET blob。当前 C-10 复审发现 in-flight 移除早于结果缓存写入，存在极窄的同 key 二次执行窗口；修复前仍以 `unknown_side_effect` 提示先重建现场。
...
```
        ...
        └──────┘                     └─────────┘            ↓
          ↑ ↑                           ↑ barge-in     ┌─────────┐
          │ │    TTS完成/打断            │(用户说话     │thinking │ 可配置主模型规划
          │ └───────────────────────────┤ 即打断TTS)   └────┬────┘
          │                             │                  ↓
      ┌───┴────┐  播报进度/结果   ┌──────┴───┐          ┌─────────┐
...
│  ├─ 1.2 模型接入
│  │  ├─ 1.2.1 主模型（M1：tool-calling + 视觉，live验证门；历史验证 GLM-4.6v）
...
│  │  ├─ 2.2.2 L1 截图通道（MediaProjection → /blob，◐ 授权回调已接·真机标定待设备）
│  │  └─ 2.2.3 L2 auto 路由（a11y优先，WebView信号已修，◐ 真机待验）
│  ├─ 2.3 操控
...
│  ├─ 4.3 测试（brain smoke 26+integration 6+专项 23；body JVM 117，快照 c57cff9）
│  └─ 4.4 验收清单（P0 §4.2）
│
...
| ActionExecutor/nonce/证据闸门 | ✅ 代码闭环 | selector 最终坐标与普通 tap 共用 typed ActionGate；敏感页真机回归待验 |
| vision/auto | ◐ 代码闭环 | attach、WebView 路由、typed fallback 已实现；授权生命周期与坐标待真机 |
| 暂停/继续/离线输入 | ◐ 代码闭环 | resume single-flight、stop 不可复活、离线发布前拒绝已实现；command ack 与真机待验 |
| 语音 | ◐ | ASR/在线播报/系统TTS有历史真机证据；本地 sherpa TTS 当前禁用 |
| 技能 | ◐ | learn/run/signature/state 已实现；仍需随安全旁路回归 |
| 记忆与归档 | ◐ | SQLite、反思、幂等归档和原子备份已实现；import 去重与真实恢复演练待验 |
| 工程验证 | ✅/◐ | brain 26 smoke+6 integration+15 resume+8 vision、body 117 JVM、Debug APK 构建通过；本轮未跑真机 |
...
| 风险 | 级别 | 缓解 |
|---|---|---|
| selector/vision/终态/暂停旧缺口回归 | 已关闭·持续监控 | typed gate、双 preflight、幂等终态、resume 专项与文档一致性测试 |
| 跨进程 command 缺少 accepted/resolved ack | P1 | 扩展 commandId 生命周期，不用本地 UI 推断接收成功 |
...
| **WP4** | 感知 L1 + 脱敏 | /blob 截图通道、可配置视觉模型感知、auto 路由、脱敏器 | WebView 页完成一次点单；密码节点出设备前已打码 | WP0,WP1 | 5d(P1窗) |
...
| ADR-7 | 感知阶梯 L0→L1→L2 | ◐ 代码闭环，真机视觉标定待验 |
...
| AD-11 | 动作统一收口 ActionExecutor | ✅ selector 最终坐标已统一过 typed ActionGate |
...
- PAUSING/STOPPING、paused checkpoint、resume single-flight 与 stopped 不可续终态已闭环；真实前后台/kill 恢复仍待真机。
- OFFLINE 已在 EventBus 前拒绝 text_input；跨进程 command accepted/rejected/resolved ack 仍未建。
