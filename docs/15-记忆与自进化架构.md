...
| ME-8 | 记忆备份导出/恢复（终审补丁：解 pm clear/换机丢记忆——§4 轮③） | ✅ 2026-08-21 | `memory.export`（全量含 revoked）+ `memory.import`（补缺语义：只插 body 缺失 active 组合，不覆盖不动 revoked，PII 同红线）；brain `memory/backup.ts`：6h 定时快照落 Termux + 启动检测空库提示恢复（不自动——尊重清空意愿）；CLI `npm run memory-backup / memory-restore`（恢复交互 y/n） |
