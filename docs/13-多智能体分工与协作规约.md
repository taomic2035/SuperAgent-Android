...
| 批次 | 审计项 | 当前任务 | Owner | Reviewer | 初始状态 |
|---|---|---|---|---|---|
| S0 | C-01 | 最终坐标闸：重叠/父子节点回归并修复所有 selector 分支 | GPT | GLM | backlog · P0 |
| S0 | C-02 | 所有感知模式 a11y preflight；敏感页 vision 不生成 blob | GPT | GLM | backlog · P0 |
| S1 | C-03/C-04 | MediaProjection attach 与 WebView auto 私有路由 | GLM | GPT | backlog · P1 |
| S2 | C-05 | 终态单写者、幂等归档、resumable 策略表 | GLM | GPT | backlog · P1 |
| S2 | C-06/C-07/C-13 | pause/resume 新 turn、离线输入 ack、状态生产者 | GPT | GLM | needs_recheck · P1 |
| S3 | C-08/C-09 | prompt parts 组合、revise 统一 PII validator | GPT | GLM | needs_recheck · P1 |
| S3 | C-11/C-14 | 私人状态忽略、原子快照、schema/checksum/权限 | GPT | GLM | needs_recheck · P1 |
| S4 | C-10 | BodyServer structured concurrency 与 unknown-side-effect | GLM | GPT | backlog · P1 |
| S4 | C-12 | act/act_done 有序事件出口 | GLM | GPT | backlog · P2 |
| S5 | C-15 | 文档事实源与测试数字持续校准 | GPT | GLM | monitoring · P2 |

### 7.1 下一步自动认领

- GPT：认领 `S0-C01-final-coordinate-gate`，先在最新 HEAD 复现/证伪 C-01。
- GLM：认领 `S1-C03-C04-vision-entry-routing`；若 GPT 正在修改共同感知文件，则先完成设计与测试用例，不覆盖同文件。
- 双方完成各自首项后交换 handoff 复核，并重新计算 S0/S1/S2 优先级。
