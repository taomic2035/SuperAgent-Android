...
| S4 | C-10 | BodyServer in-flight→cache 原子 handoff 与 unknown-side-effect | GLM | GPT | needs_recheck · P1 |
| S4 | C-12 | act/act_done 有序事件出口 | GLM | GPT | closed |
| S5 | C-15 | 文档事实源与测试数字持续校准 | GPT | GLM | monitoring · CI guard |

### 7.1 下一步自动认领

- GPT：`BR-10/CT-06` UX/安全审计与设计已冻结，P0 状态倒退回归已完成；下一步认领 P1 共享协议枚举、回执语义与 UI 验证。
- GLM：优先完成 `S4-C10-idempotency-handoff`；随后认领 command-ack P2/P3（Body journal、Brain command pump、持久 cursor），与 GPT 在共享契约/main 接线前同步 claim。
