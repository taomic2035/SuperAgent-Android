...
当前限制：产品入口尚未把 MediaProjection 授权结果 attach 到 ScreenshotService；显式 `vision` 的敏感前台预检顺序也待修；WebView 信号没有可靠传到 auto 路由。因此“协议可表达 vision”不等于“视觉链已完成”。
...
安全不变量：任何最终坐标手势都应经过 `ActionGate.checkCoordinate`。当前 `selectOption` 的非校验分支仍有内部 tap 旁路（docs/16 C-01），故本表描述的是目标契约，不代表所有实现路径已满足。
...
| method | params | result | 说明 |
|---|---|---|---|
| run.archive | `{id,goal,startedAt,finishedAt,outcome,failureReason?,traceJson}` | `{archived:true}` | SQLite 全量归档；当前终态多写可能重复调用（docs/16 C-05） |
