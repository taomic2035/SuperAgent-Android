...
|---|---|---|
| success | `kind=vision`；仅表示输出语法、有限值和截图/屏幕边界校验通过 | **规范要求：否；动作入口必须 fail-closed** |
...
当前 MediaProjection attach、WebView 结构化信号和 C-02 敏感前台双刷新门禁已闭环；但 VLM 输出坐标系尚未通过受控设备标定，`success` 不代表语义定位正确。**规范要求**在设备动作前对 direct-coordinate fail-closed；**当前代码仍可能暴露或消费视觉 marks，此门禁尚未闭合，是待修代码 gap**，不得描述为已经禁用。SoM 编号徽章只是首选候选方案，仍须实现与真机验证后才能解除该限制。
...
```
BrainEvent {
  taskId: string          // 非空；任务隔离（含 boot-session ID，brain 重启安全）
  seq: number             // >=0，单调递增（UI 去重/排序）
  state: string           // 有限枚举：prompt_start/act/act_done/hitl_wait/blocked/finish/error/heartbeat
  stepIndex?: number      // 已发生步骤序号
  displayText?: string    // 默认空串；已脱敏用户可读（≤40 字），禁坐标/工具名/JSON
  requiresUser?: string   // 默认 none；有限枚举 none/control/confirm/handoff
  resultKind?: string     // finish 必填，非 finish 禁止；success/failed/aborted/paused/stopped/unknown_side_effect
  timestamp: number
  sourceSessionId?: string // P1 兼容关联字段；生产与水位语义在后续阶段接线
  commandId?: string       // P1 兼容关联字段；当前不表示已 accepted/resolved
}
```

`brain.event` 在入站边界执行上述结构与跨字段校验；任一约束失败返回 `BAD_PARAMS`，且不得向 EventBus 发布事件。`displayText`、`requiresUser` 保持可选是为了兼容旧发送端，缺省后分别规范化为 `""` 与 `none`。

`state` 与 `requiresUser` 的最小合法组合如下；未列出的组合均应 fail-closed：

| `state` | 允许的 `requiresUser` | 说明 |
|---|---|---|
| `hitl_wait` | `confirm` | 等待用户确认 |
| `blocked` | `control` / `handoff` / `none` | `none` 仅作为旧发送端兼容值 |
| `prompt_start` / `act` / `act_done` / `finish` / `error` / `heartbeat` | `none` | 不得暗含额外用户操作 |
