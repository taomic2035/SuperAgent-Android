...
| TIMEOUT | 200 | handler 30s 超时 | 可安全重试（幂等键不变） |
| BLOB_UNSUPPORTED（预留） | 200 | P1 前访问 /blob | 等升级 |

## 2. RPC 方法全量规格

坐标约定：**归一化 0–999**（body 侧换算像素）。`near` 为同文多匹配时就近消歧参考点。
...
```
TraceStep { tool:string, args?:map, located:boolean, signature?:string, timestamp:long }
```
...
| method | params | result | errors |
|---|---|---|---|
| hitl.confirm | `{prompt:string}` | `{approved:boolean}`（60s 超时=false） | — |
...
| brain session 落盘 | pi-agent-core session 格式 + runState{goal, baselineScreen, trace[]} |
